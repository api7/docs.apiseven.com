...
* **[AI Proxy Multi](/hub/ai-proxy-multi)**
...
* 修复问题：新增支持在 Stream 类型服务上游中配置 `retry_timeout` 和 `retries` 参数。

#### 数据面

* 修复问题：独立模式下使用大型配置文件时启动时间过长。
* 修复问题：[OAS Validator](/hub/oas-validator) 插件错误拒绝 1.11-1.19 范围内符合 `multipleOf: 0.01` 规则的数字。
* 修复问题：优化 [AI Proxy](/hub/ai-proxy) 插件的 schema 验证。
* 修复问题：standalone 模式下使用大体积型配置文件时启动时间过长。
...
* 修复问题：[OAS Validator](/hub/oas-validator)插件错误拒绝1.11-1.19范围内符合`multipleOf: 0.01`规则的数字。
