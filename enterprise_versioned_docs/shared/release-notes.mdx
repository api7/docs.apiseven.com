...
* **[openapi-to-mcp](https://docs.api7.ai/hub/openapi-to-mcp)**
  * 当服务中配置了 `path_prefix` 和 `strip_path_prefix` 时，sse 模式不工作
  * 使用 openapi 的操作 ID 作为 mcp 工具名称
  * 如果未配置 `headers` 字段，将会抛出 500 状态码错误
* 如果存在相似的网关组名称，ADC 同步可能会同步到错误的网关组上
* 登录选项角色映射如果没有匹配任何规则，不应保留以前的角色
* OIDC 登录选项请求范围字段中移除添加按钮
* 偶尔无法在编辑登录选项表单中显示旧配置
* 简化核心资源架构以避免 ADC diff 命令输出不必要的更改
* 更新用户角色时更新用户的 `updated_at`
...
* **[OpenAPI to MCP](/hub/openapi-to-mcp)**
