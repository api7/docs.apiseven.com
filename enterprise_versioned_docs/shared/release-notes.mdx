...
* 全新设计的 Developer Portal。
  * 提供开源 SDK 和前端脚手架项目，方便用户二次开发。
  * 新增 Portal 级别认证机制用于对接 API。
  * 移除内置的 Portal SSO 功能。
* `limit-conn` 和 `limit-req` 插件新增必填字段：`policy`，已有配置无须修改，在数据面可以正常工作，但更新配置时需要补充该字段 (`policy=local`)，否则更新会被拒绝。
...
* **[Loki Logger](https://docs.api7.ai/hub/loki-logger/)**
  * 支持自定义发送给 loki server 的 http header。
* **File Logger**
  * 支持通过 `match` 字段匹配指定的请求进行日志记录。
...
* 允许在开启 SSO 登录后完全禁用内置的用户名密码登录。
* 支持配置数据库执行语句的最大执行时间。
* **可观测性优化**
  * 默认开启 pprof 性能分析功能。
  * metrics 端点新增数据库连接池监控指标。
  * 支持配置访问日志和错误日志分开记录。
...
* 修复问题：优化 DNS 解析结果同时包含 CNAME 和 A 记录时的缓存行为。

#### 控制面 {#390-缺陷修复-控制面}

* 修复问题：移除网关实例列表中 IP 和 Port 展示，避免误导用户。
* 修复问题：在 API 调用次数统计表并发批量插入时可能出现数据库死锁的情况。
* 修复问题：在 PostgreSQL 中使用非 `public` schema 时控制台启动失败的问题。
...
* **[AI Rate Limiting](https://docs.api7.ai/hub/ai-rate-limiting/)**
  * 支持使用 Redis、Redis Cluster、Redis Sentinel 作为存储。
    ...
    `ai-rate-limiting` 插件新增必填字段：`policy`。已有配置无须修改，在数据平面可以正常工作；但更新配置时需要补充 `policy=local` 字段，否则更新会被拒绝。
