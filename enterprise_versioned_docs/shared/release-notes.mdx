...
* **[AI Aliyun Content Moderation](/hub/ai-aliyun-content-moderation)**  
  * 新增对流式（HTTP SSE）场景的支持。
* **[AI Prompt Guard](/hub/ai-prompt-guard)**  
  * 新增插件。
* **[Kafka Logger](/hub/kafka-logger)**
  * 支持读取缓存在 `ctx` 中的用户请求和响应体内容，并将其推送到指定的 Kafka 服务。

#### 控制台

* 在状态码告警消息中添加了服务名称和路由名称的详细统计信息。
* 优化插件配置：在服务或路由上配置 [Traffic Split](/hub/traffic-split) 插件时，验证配置的 `upstream_id` 确实属于当前服务的上游。
* 在已发布服务中将上游连接配置字段名称与 API 对齐。
* 为 [CORS](/hub/cors) 插件配置引入了基于表单的 UI。
...
* **[OpenTelemetry](/hub/opentelemetry)**
...
* **[OpenID Connect](/hub/openid-connect)**
  * 修复问题：无法配置验证颁发者。
...
* **[AI Rate Limiting](/hub/ai-rate-limiting)**
  * 引入新的 AI 插件，用于对发送到 LLM 服务的请求实施基于令牌的速率限制。它通过控制指定时间范围内消耗的令牌数量来管理 API 使用，确保资源公平分配并防止服务过载。通常与 [AI Proxy Multi](/hub/ai-proxy-multi) 插件一起使用。
* **[AI Proxy](/hub/ai-proxy)**
  * 支持 openai 兼容的提供者。
  * 支持代理嵌入模型 API。
* **[AI Proxy Multi](/hub/ai-proxy-multi)**
...
* **[Limit Count Advanced](/hub/limit-count-advanced)**
...
* **[Elasticsearch Logger](/hub/elasticsearch-logger)**
