...
为支持创建多个 Portal 实例，数据面的 Portal 认证插件（`portal-auth`）进行了升级。用户在升级控制面后应尽快升级数据面，在此期间请勿更新已有的 API 产品，否则这部分变更在升级数据面前将无法生效。
...
* 修复问题：Ingress Controller 安装脚本中锁定 Helm Chart 版本号。
* 修复问题：处于离线状态的网关实例移除兼容性标签。
