...
要主动监控即将过期的 SSL 证书并发出告警，可以配置一个每日告警任务以检查证书到期日期。如果证书即将到期（30 天内），请向应急小组发送邮件告警，并在 Slack 上发布通知。
...
3. 在对话框中，执行以下操作：
   ...
   * 在对话框中，执行以下操作：
      * **类型**，选择 `邮件`。
      * **联系人**，选择 `应急小组邮件列表`。
      * **告警邮件主题**，输入 `[API7 告警] [{{.Severity}}] SSL 证书到期警告`。
...
假设网关组 `生产网关组` 中的 SSL 证书将于 2024-12-31 过期。
...
2. 你应该会看到一个告警记录。点击**详情**：
   ...
   * 告警时间：5 分钟前
   * 触发网关组：生产网关组
   * 告警详情：Certificate will expire in 21 days.

3. 发送的邮件将类似于：

```text
* 主题：[API7 告警][高] SSL 证书到期警告
* 告警时间：2024 DEC 10 17:00:00，详情：网关实例证书：Certificate will expire in 21 days.
```

4. 你将在 Slack 中收到一条消息：

```text
Certificate will expire in 21 days.
```

### 监控控制面到数据面的 mTLS 证书到期

API7 控制面证书和 API7 控制面 CA 证书支持控制面和数据面之间安全的 mTLS 通信，并在实例部署时激活，有 13 个月的有效期。

要主动监控即将过期的网关实例证书并发出告警，请配置每日告警任务以检查证书到期日期。如果证书即将到期（30 天内），请向应急小组发送邮件告警，并在 Slack 上发布通知。
...
假设网关组 `生产网关组` 中的网关实例 `gateway 123` 的证书将于 2024-12-31 过期。
...
```text
* 主题：[API7 告警] 网关实例证书到期警告
* 告警时间：2024 DEC 10 17:00:00，详情：Certificate of gateway instance: gateway 123 will expire in 21 days.
```
