...
对于高级场景，例如金丝雀部署、蓝绿部署或管理多个集群，服务现在可以使用多个上游。在这种情况下，默认上游充当大多数请求的主要目标，而其他上游可用于特定目的，例如将流量路由到金丝雀部署或辅助集群。有关详细信息，请参阅更新后的[配置金丝雀流量转移](./getting-started/canary-upstream.md)。

:::info

旧的 **Canary Rule** 功能不再可用。
