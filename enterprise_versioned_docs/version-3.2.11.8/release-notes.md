...
- 通过减少对 etcd 的调用提高 API 性能。
- 修复了 Kine 数据库连接池配置异常的问题。
...
- 进一步提升 API 的性能。
- 数据面支持关闭遥测数据收集，以及配置上报时间间隔。
- 自定义插件在 schema 为空时也不会报错影响运行。
...
- 修复了 ssl_verify 配置在登陆选项 OIDC 和 LDAP 中可能不生效的问题。
